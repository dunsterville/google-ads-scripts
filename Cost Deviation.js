var COST_PRECENT_DIFFERENCE = 0.10; // Precent difference 0.10 = 10%
...
function send_account_notification() {
  ...
  while (rowsStart.hasNext()) {
    ...
    var CostStart = row['Cost']
    CostStart = parseFloat(CostStart)
  }
  while (rowsEnd.hasNext()) {
    var row = rowsEnd.next();
    var CostEnd = row['Cost']
    CostEnd = parseFloat(CostEnd)
    var account_name = row['AccountDescriptiveName']
  }  
  if (Math.abs((CostStart - CostEnd) / ((CostStart + CostEnd) / 2)) > COST_PRECENT_DIFFERENCE) {
    Logger.log(" Cost change is below threshold.. send email")
    sendMail(account_name, (CostStart - CostEnd) / ((CostStart + CostEnd) / 2)*100)
  }
  else {
    Logger.log(" Cost change is above threshold.. do not send email..")
  }
  ...
  function sendMail(account_name, PrecentDifference) {
    ...
    html.push(
      ...
      "<tr bgcolor='#3c78d8′>",
      "<td width=500>",
      "<div style='font: normal 18pt verdana, sans-serif; padding: 3px 10px; color: white'>Cost Precent Difference Alert</div>",
      "</td>",
      "<td align=right>",
      ...
      "<tr bgcolor='#ddd'>",
      "<td style='font: 12pt verdana, sans-serif; padding: 5px 0px 5px 5px; background-color: #ddd; text-align: center'>Cost has changed by: " + PrecentDifference + "%</td>",
      "</tr>",
      "</table>",
      "</body>",
      "</html>");
    MailApp.sendEmail(Email_ids, 'KB Cost Precent Difference Threshold Script', "", { htmlBody: html.join("\n") });
  }
  //*****************************************************************************
